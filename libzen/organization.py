...
def create(**organization_props) -> int:
    ...
    return next(
        _send('/api/v2/organizations.json', data, result_page_name='organization', method='post')
    )['id']
...
def update(organization_id: 'Union[str, int]', **organization_props) -> dict:
    ...
    return next(
        _send(
            '/api/v2/organizations/' + str(organization_id),
            data,
            result_page_name='organization',
            method='put',
        )
    )
