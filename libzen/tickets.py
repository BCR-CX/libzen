...
def update(ticket_id: 'Union[str, int]', **ticket_props) -> dict:
    ...
    data = json.dumps({'ticket': ticket_props})
    return next(
        _send('/api/v2/tickets/' + str(ticket_id), data, result_page_name='ticket', method='put')
    )
...
def append_tags(ticket_id: 'Union[str, int]', tags: 'list[str]') -> int:
    ...
    return next(
        _send(
            f'/api/v2/tickets/{ticket_id}/tags',
            data,
            headers=headers,
            result_page_name='tags',
            method='put',
        )
    )
