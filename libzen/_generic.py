...
def _send(endpoint:str, content_object:Any, result_page_name:str, headers=None, method:str='post'):
    ...
    response = _METHOD[method](full_url, data=content_object, auth=_Authentication.authentication.to_tuple(), headers=headers)
...
def _delete(endpoint: str, result_page_name: str='results'):
    ...
    response = requests.delete(full_url, auth=_Authentication.authentication.to_tuple())
...
def _iterate_search(endpoint: str, result_page_name: str='results'):
    ...
    while True:
        response = requests.get(nextpage, auth=_Authentication.authentication.to_tuple())
